...
plotEnrichment <- function(pathway, pname, stats, gseaParam = 1, ticksSize = 0.2){
  ...
  g3 = ggplot(data = rank_mat, aes(x = ranks, y = h, fill = score))+geom_tile()+gstyle+midstyle+
    ...
    xlab('')+ylab('')+
    scale_fill_gradient2(high = 'red', low = 'blue', mid = 'white', midpoint = sort(rank_mat[rank_mat$score > 0, 'score'])[1])
  # distribution show in barplot
