...
def geneROC(genes, rp_dict, lgfc, fdr):
    ...
    genes_symbol_all = list(rp_dict.keys())
    ...
    return list(gene_rp_dict.values())
...
def calcAUC(genes, rp_dict, lgfc, fdr):
    ...
    for i in genes_symbol_sig:
        gene_rp_dict[i] = [i, rp_dict[i], 1]
    gene_rp_de_list = list(gene_rp_dict.values())
    TP = 0.0
    FP = 0.0
    TPR = [0.0]
    FPR = [0.0]
...
def calRPTrank(rank_product_list):
    ...
    rank_product_rpt = [np.sqrt(rank_product_fdr_rank[i]*rank_product_RP_rank[i]) for i in range(len(rank_product_list))]
...
def geneClassify(expr_dict, rp_dict, dego):
    ...
    for i in list(rp_dict.keys()):
        ...
        try:
            ...
            # expr_dict.pop(i)
    df = pd.DataFrame(rank_product_dict, index = ['gene', 'Log2FC', 'FDR', 'normRP']).T

    # rank_product_genes = rank_product_dict.keys()
    rank_product_list = list(rank_product_dict.values())
    res_rank_product = {}
    symbol_rpt = {}
    if dego == "allgenes":
        ...
        for i in rank_product_list:
            ...
            symbol_rpt[i[0]] = i[-2]
        genes_maxl = list(symbol_rpt.keys())
        ...
        # with open('test2.pk', 'wb') as out:
        #     pk.dump(rank_product_list_down, out)
    ...
    ## output rank
    df['rankproduct'] = pd.Series(symbol_rpt)[df.index.tolist()]
    # with open('test.pk', 'wb') as out:
    #     pk.dump(res_symbol_rpt, out)
...
def calcRPT(expr_path, expr_info, symbol_rp_dict, assembly, dego, logfc_cut, fdr_cut):
    ...
    symbol_rpt_dict, df = geneClassify(expr_dict, symbol_rp_dict["scores"], dego)
    return symbol_rpt_dict, df, res_rate_dict
