    ...
    with open(file_path, "rU") as pfile:
        ...
        for line in lines:
            if line.startswith("#"):
                ...
                first_line = line
                last_line = lines[int(len(lines)/2)]
                break
...
def checkExpr(file_path, expr_info):
    ...
    for sep in ["\t", ",", " "]:
        ...
        with open(file_path, "rU") as exprfile:
            ...
            if len(first_line) == 1:
                continue
            last_line = lines[int(len(lines)/2)].strip().split(sep)
...
def transGeneID(expr_list, assembly):
    ...
    genes_list_correct = set([x.replace("_at", "").split(".")[0] for x in genes_list])
    genes_list_length = float(len(genes_list))
    geneids_intersect = {}
    for i in list(geneids_all.keys()):
    ...
    genesid_match = max(iter(geneids_intersect.items()), key=operator.itemgetter(1))[0]
...
def readExpr(expr_path, expr_info, assembly):
    ...
    # for i in expr_list_symbol:
        ...
        #     else:
        #         expr_dict[i[0]] = [i[0], 1.0]
    upgenes = [i for i in list(expr_dict_symbol.values()) if i[1]>=0]
    downgenes = [i for i in list(expr_dict_symbol.values()) if i[1]<=0]
    Info("Finish reading expression file. %s genes in total. %s upregulated genes. %s downregulated genes. "
         % (len(list(expr_dict_symbol.values())), len(upgenes), len(downgenes)))
