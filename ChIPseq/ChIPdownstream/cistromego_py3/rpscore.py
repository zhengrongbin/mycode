...
def propCalc(genes_list, peaks_list):
    ...
    for top_peak_number in [2000, 5000, 10000, 1000000]:
        ...
        peak_within_1k = [i for i in list(D.values()) if i <=1000]
...
def peaksInRange(genes_list, peaks_list, decay):
    ...
    peak_within_padding = [i for i in list(D2.values()) if i <= padding]
...
def getRP(D, assembly, decay):
    ...
    for gene_name in D:
        RP[gene_name] = sum([2.0 ** x for x in [-x / decay for x in D[gene_name]]])
    for symbol in symbol_ref:
        if len(symbol_ref[symbol]) == 1:
            ...
            ref_maxrp = max(iter(symbol_rp_tmp.items()), key=operator.itemgetter(1))[0]
            res_table_data.append(
    ...
    for i in range(len(res_table_data)): ### NOTE: change the decimals of RPscore here
        res_table_data[i] += [res_table_adjrp[i], res_table_adjrp_rank[i]]
    ...
    genes_maxl = [i for i in list(symbol_rp.keys()) if symbol_rp[i]>0]
    res_symbol_rp = {}
...
def calcRPscore(bed_path, peakn, assembly, decay):
    ...
    return res_rp_table, symbol_rp_dict, decay
